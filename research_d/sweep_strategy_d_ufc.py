"""UFC sweep — parallel parameter search for Strategy D UFC variant.

Uses prepare_ufc.py. Similar structure to sweep_strategy_d.py (NBA) but
with UFC-specific parameter ranges based on sport characteristics:
  - Larger green_book_delta (UFC has bigger per-round swings)
  - Wider max_edge (UFC market is less efficient than NBA)
  - Shorter max_hold (fights are shorter)

Usage:
    PYTHONPATH=. python3 research_d/sweep_strategy_d_ufc.py --worker 1 --n-workers 3
"""
...
from research_d.prepare_ufc import DEFAULT_DB_PATH, DEFAULT_PARAMS, evaluate

RESULTS_FILE = Path(__file__).parent / "data" / "sweep_ufc_results.tsv"


def build_param_grid():
    """UFC-focused grid. Based on sport characteristics from spec."""
    base = dict(DEFAULT_PARAMS)
    combos = []
    for me in [0.05, 0.08, 0.10, 0.12, 0.15]:
        for gbd in [0.15, 0.20, 0.25, 0.30]:

...
Secret masking processor ensures passwords/tokens/private keys never leak into logs.
"""
...
_SECRET_PATTERNS = re.compile(
    ...
    r"|private[-_]?key|passphrase|mnemonic)",
    re.IGNORECASE,
)
_MASK = "***REDACTED***"
...
def _mask_secrets(
    ...
    for key in list(event_dict.keys()):
        if _SECRET_PATTERNS.search(key):
            ...
            val = event_dict[key]
            # Mask values that look like known token formats
            if val.startswith(("xoxb-", "xapp-", "sk-", "sk-ant-", "0x")):
                event_dict[key] = val[:6] + "..." + _MASK
    return event_dict
...
def setup_logging(log_level: str = "INFO", json_output: bool | None = None) -> None:
    ...
    for noisy in ("aiohttp", "asyncio", "sqlalchemy.engine", "websockets", "web3"):

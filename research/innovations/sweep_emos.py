"""Autoresearch Sweep: EMOS Model vs Baseline (AR-0134).

Optimizes EMOS-based probability model on real Polymarket price history.
Compares against baseline (fixed Normal CDF with per-city sigma/bias).

Key innovations being tested:
  1. Adaptive sigma from recent forecast errors (rolling RMSE/MAE/EWMA)
  2. Adaptive bias correction (rolling mean / EWMA)
  3. Sigma floor & scale as tunable parameters
  4. Walk-forward: EMOS trains only on past resolved events (no look-ahead)

Data source: price_history.sqlite (same as sweep_real_prices.py)
Scoring: same composite score formula for fair A/B comparison.

Usage:
    python3 research/innovations/sweep_emos.py
"""
...
RESULTS_PATH = RESULTS_DIR / "sweep_emos_results.json"
LOG_PATH = Path(__file__).parent.parent / "sweep_emos_log.txt"
...
# ── Build training observations per city ─────────────────────────────


def _build_city_observations(
    db: PriceHistoryDB,
    events: list,
) -> dict[str, list[Observation]]:
    """Build (forecast, actual) pairs per city from resolved events.

    The 'actual' temperature is estimated from the winning bucket midpoint.
    Ordered by target_date for walk-forward splitting.
    """
    ...
    for ev in events:
        ...
        # Get actual temperature from winning bucket
        snap = db.get_price_snapshot(ev.event_id, hours_before_close=24)
        if not snap:
            continue
...
def evaluate_params(
    db: PriceHistoryDB,
    events: list,
    city_observations: dict[str, list[Observation]],
    params: dict,
    entry_hours: float = 24,
) -> dict:
    """Evaluate EMOS parameter set on real price data.

    Walk-forward: for each event, EMOS is fit only on prior resolved events.
    """
    # EMOS model params
    emos_training_window = params.get("emos_training_window", 30)
    emos_sigma_method = params.get("emos_sigma_method", "rolling_rmse")
    emos_bias_method = params.get("emos_bias_method", "rolling_mean")
    emos_sigma_floor = params.get("emos_sigma_floor", 0.5)
    emos_sigma_scale = params.get("emos_sigma_scale", 1.0)
    emos_ewma_alpha = params.get("emos_ewma_alpha", 0.1)

    # Quality gate params (same as baseline)
    ...
    # Pre-build EMOS model template
    emos_template_params = dict(
        training_window=emos_training_window,
        sigma_method=emos_sigma_method,
        bias_method=emos_bias_method,
        sigma_floor=emos_sigma_floor,
        sigma_scale=emos_sigma_scale,
        ewma_alpha=emos_ewma_alpha,
    )
    ...
    for ev in events:
        ...
        # ── Walk-forward EMOS fit ──
        # Get all observations BEFORE this event's date
        city_obs = city_observations.get(ev.city, [])
        prior_obs = [o for o in city_obs if o.date < ev.target_date]

        # Fit EMOS on prior observations
        model = EMOSModel(**emos_template_params)
        model.fit(prior_obs)
        ...
        for bucket in snap.buckets:
            ...
            # ── EMOS probability ──
            raw_prob = model.bucket_probability(
                forecast_temp,
                bucket.low_c,
                bucket.high_c,
            ...
            # Apply shrinkage (same as baseline)
            raw_prob = raw_prob * (1.0 - shrinkage) + 0.125 * shrinkage

            # Apply sharpening (same as baseline)
            ...
            trades.append(
                {
                    ...
                    "size": size,
                    "date": ev.target_date,
                    "emos_sigma": model.sigma,
                    "emos_bias": model.bias,
                    "emos_n_obs": model.n_observations,
                }
            )
    ...
    if len(daily_vals) >= 2:
        ...
        var_d = sum((d - mean_d) ** 2 for d in daily_vals) / (
            len(daily_vals) - 1
        )
    ...
    # EMOS diagnostics
    avg_sigma = (
        sum(t["emos_sigma"] for t in trades) / n_trades if n_trades else 0
    )
    avg_n_obs = (
        sum(t["emos_n_obs"] for t in trades) / n_trades if n_trades else 0
    )

    # Composite score (same formula as sweep_real_prices.py for fair comparison)
    ...
    return {
        ...
        # EMOS diagnostics
        "avg_emos_sigma": round(avg_sigma, 3),
        "avg_emos_n_obs": round(avg_n_obs, 1),
    }
...
def random_params(rng: random.Random) -> dict:
    """Generate random parameter set — EMOS + quality gate."""
    return {
        # EMOS model params
        "emos_training_window": rng.choice([7, 10, 14, 21, 30, 45, 60]),
        ...
        # Quality gate params (same search space as baseline)
        "min_edge": rng.choice(
        ...
        "min_price": rng.choice(
            [0.02, 0.03, 0.05, 0.08, 0.10, 0.12, 0.15, 0.20]
        ),
        "min_prob": rng.choice(
            [0.02, 0.05, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30]
        ),
        ...
        "shrinkage": rng.choice(
            [0.0, 0.01, 0.03, 0.05, 0.08, 0.10, 0.15]
        ),
        "excluded_cities": set(),
        "city_overrides": {},
    }
...
def perturb_params(base: dict, rng: random.Random) -> dict:
    ...
    # All perturbable keys (EMOS + quality gate)
    emos_keys = [
    ...
    for key in rng.sample(all_keys, min(n_perturb, len(all_keys))):
        ...
        if key == "emos_training_window":
            ...
            p[key] = round(
                max(0.1, val + rng.choice([-0.2, -0.1, 0.1, 0.2])), 2
            )
        elif key == "emos_sigma_scale":
            p[key] = round(
                max(0.3, val + rng.choice([-0.2, -0.1, 0.1, 0.2])), 2
            )
        elif key == "emos_ewma_alpha":
            p[key] = round(
                max(0.01, min(0.5, val + rng.choice([-0.05, 0.05]))), 2
            )
        elif key == "min_volume":
            p[key] = max(
                0, val + rng.choice([-200, -100, -50, 50, 100, 200])
            )
        elif key == "prob_sharpening":
            p[key] = round(val + rng.choice([-0.10, -0.05, 0.05, 0.10]), 2)
        elif key == "shrinkage":
            p[key] = round(
                max(0, val + rng.choice([-0.03, -0.02, 0.02, 0.03])), 2
            )
            ...
            p[key] = round(
                max(
                    0.01,
                    val
                    + rng.choice([-0.03, -0.02, -0.01, 0.01, 0.02, 0.03]),
                ),
                2,
            )

    # Occasionally flip sigma_method or bias_method
    if rng.random() < 0.15:
        p["emos_sigma_method"] = rng.choice(
            ["rolling_rmse", "rolling_mae", "ewma_rmse"]
        )
    if rng.random() < 0.15:
...
def evaluate_baseline(
    ...
    """Evaluate BASELINE (fixed sigma) with the same quality gate params.

    This runs the exact same logic as sweep_real_prices.py evaluate_params
    so we get a fair A/B comparison with identical gate + sizing params.
    """
    min_edge = params["min_edge"]
    max_price = params["max_price"]
    min_price = params["min_price"]
    ...
    kelly_raw_cap = params.get("kelly_raw_cap", 0.40)
    prob_sharpening = params.get("prob_sharpening", 1.05)
    shrinkage_val = params.get("shrinkage", 0.0)
    excluded = params.get("excluded_cities", set())
    city_overrides = params.get("city_overrides", {})
    ...
    for ev in events:
        ...
        for bucket in snap.buckets:
            ...
            raw_prob = raw_prob * (1.0 - shrinkage_val) + 0.125 * shrinkage_val
            if prob_sharpening != 1.0 and raw_prob > 0:
                raw_prob = raw_prob**prob_sharpening
            ...
            max_dd = max(max_dd, dd)
            daily_pnl[ev.target_date] += pnl
            trades.append({"won": bucket.won, "pnl": pnl, "city": ev.city})

    n_trades = len(trades)
    if n_trades == 0:
        return {"trades": 0, "pnl": 0, "score": 0}
    ...
    for t in trades:
        city_pnl_map[t["city"]] += t["pnl"]
    unprofitable = [c for c, p in city_pnl_map.items() if p < 0]

    pnl_factor = total_pnl / INITIAL_CAPITAL
    dd_factor = max(0, 1.0 - max_dd * 2)
    trade_factor = min(n_trades / 50, 2.0)
    city_penalty = max(0, 1.0 - len(unprofitable) * 0.1)
    sharpe_factor = min(max(sharpe, 0) / 5.0, 2.0)
...
def main():
    ...
    city_observations = _build_city_observations(db, events)
    ...
    log(f"\n{'=' * 70}")
    log(f"EMOS AUTORESEARCH SWEEP")
    log(f"Events: {len(events)}, Start: {datetime.now().isoformat()}")
    log(f"Training obs per city: {', '.join(f'{c}={len(o)}' for c, o in sorted(city_observations.items()))}")
    log(f"{'=' * 70}\n")

    # ── Run baseline for reference ──
    log("Running baseline (AR-0134 params) for comparison...")
    baseline_params = {
        "min_edge": 0.01,
        "max_price": 0.55,
        "min_price": 0.08,
        ...
        "shrinkage": 0.0,
        "excluded_cities": {"chicago", "tel_aviv", "paris", "london"},
        "city_overrides": {
            "sao_paulo": {"max_price": 0.6, "min_edge": 0.03},
            "seoul": {"max_price": 0.6, "min_edge": 0.05},
            "seattle": {"max_price": 0.55, "min_edge": 0.05},
            "ankara": {"max_price": 0.5, "min_edge": 0.01},
            "miami": {"max_price": 0.5, "min_edge": 0.03},
            "tokyo": {"max_price": 0.6, "min_edge": 0.02},
        },
    }
    baseline_result = evaluate_baseline(db, events, baseline_params)
    log(
        f"  Baseline: score={baseline_result['score']:.2f}  "
        f"trades={baseline_result['trades']}  "
        f"PnL=${baseline_result['pnl']:.2f}  "
    ...
    n_phase1 = 600
    log(f"Phase 1: Random Search ({n_phase1} trials)")
    log("-" * 50)

    for trial in range(n_phase1):
        params = random_params(rng)
        result = evaluate_params(db, events, city_observations, params)
        result["params"] = {
        ...
        if result["score"] > best_score:
            ...
            best_result = result
            log(
                f"  Trial {trial:>3}: score={result['score']:>8.2f}  "
                f"trades={result['trades']:>3}  "
                f"WR={result['win_rate']:>5.1f}%  "
                f"PnL=${result['pnl']:>8.2f}  "
                f"DD={result['max_dd']:>5.2f}%  "
                f"sigma={result.get('avg_emos_sigma', 0):.2f}  "
                f"★ NEW BEST"
            )

        if (trial + 1) % 100 == 0:
            log(
                f"  ... {trial + 1}/{n_phase1} done, "
                f"best score: {best_score:.2f}"
            )

    # ── Phase 2: Fine-tuning around top 10 (400 trials) ──
    n_phase2 = 400
    log(f"\nPhase 2: Fine-tuning ({n_phase2} trials around top 10)")
    log("-" * 50)

    phase1_sorted = sorted(all_results, key=lambda r: -r["score"])[:10]
    log(f"  Top 10 scores: {[r['score'] for r in phase1_sorted]}")

    trial_offset = n_phase1
    for i in range(n_phase2):
        ...
        if result["score"] > best_score:
            ...
            best_result = result
            log(
                f"  Trial {trial_offset + i}: score={result['score']:>8.2f}  "
                f"trades={result['trades']:>3}  "
                f"PnL=${result['pnl']:>8.2f}  "
                f"sigma={result.get('avg_emos_sigma', 0):.2f}  "
                f"★ NEW BEST"
    ...
    n_phase3 = 200
    log(f"\nPhase 3: City optimization ({n_phase3} trials)")
    log("-" * 50)
    ...
    trial_offset = n_phase1 + n_phase2
    for i in range(n_phase3):
        params = dict(best_params)
        params["excluded_cities"] = set(
            params.get("excluded_cities", set())
        )
        params["city_overrides"] = dict(
            params.get("city_overrides", {})
        )
        ...
        if result["score"] > best_score:
            ...
            log(
                ...
                f"trades={result['trades']:>3}  "
                f"PnL=${result['pnl']:>8.2f}  "
                f"excluded={list(params['excluded_cities'])}  "
                f"★ NEW BEST"
            )
    ...
    # ── Final report ──
    log(f"\n{'=' * 70}")
    log(f"EMOS SWEEP COMPLETE — {elapsed / 60:.1f} minutes")
    log(f"{'=' * 70}")

    log(f"\n  ┌─ BASELINE (fixed sigma, AR-0134 params) ─┐")
    log(
        f"  │ Score: {baseline_result['score']:.2f}  "
        f"Trades: {baseline_result['trades']}  "
        f"PnL: ${baseline_result['pnl']:.2f}  │"
    )
    log(f"  └──────────────────────────────────────────┘")
    ...
    delta = best_result["score"] - baseline_result["score"]
    pnl_delta = best_result["pnl"] - baseline_result["pnl"]
    log(
        f"\n  EMOS vs Baseline delta: "
        f"score {'+' if delta >= 0 else ''}{delta:.2f}, "
        f"PnL {'+' if pnl_delta >= 0 else ''}${pnl_delta:.2f}"
    )

    log(f"\n  Best EMOS parameters:")
    ...
    for k in [
        ...
        log(f"    {k}: {bp.get(k, 'N/A')}")
    log(f"    avg_sigma (fitted): {best_result.get('avg_emos_sigma', 'N/A')}")
    log(f"    avg_n_obs (training): {best_result.get('avg_emos_n_obs', 'N/A')}")
    log(f"    --- Quality gate ---")
    for k in [
        "min_edge",
        "max_price",
    ...
    for i, r in enumerate(top5):
        log(
            f"    #{i + 1}: score={r['score']:.2f}  trades={r['trades']}  "
            f"WR={r['win_rate']}%  PnL=${r['pnl']:.2f}  "
            f"sigma={r.get('avg_emos_sigma', 0):.2f}  "
            f"(trial {r['trial']}, phase {r['phase']})"
        )

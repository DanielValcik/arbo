# ARBO — Development Brief v3.0 FINAL
...
|---|---|---|---|---|---|
| Matchbook | ✅ accessible | 4% net win | REST (free <1M GET/mo) | P1 | Primary automated execution |
...
| Polymarket | ✅ accessible | 2% winner fee | py-clob-client SDK | P3 | Cross-domain arb supplement |
...
| LLM | Claude Haiku 4.5 API | News analysis, structured extraction |
...
| News Aggregator | RSS + Reddit + NewsAPI | 5 min | Deduplicate by SHA-256 hash of title+body. Store in `news_items`. |
...
| Situational Edge | P3 | Claude Haiku analyzes news for upcoming events. Structured JSON output: `{event_id, impact, magnitude: 1-10, direction, confidence: 0-1}` | Confidence > 0.7 AND magnitude > 5 |
...
1. **Matchbook Executor:** `POST /offers` → poll status → cancel unmatched after 30s → handle partial fills (accept >60%, cancel <60%) → log to `bets` table.
...
CREATE TABLE news_items (
    id              SERIAL PRIMARY KEY,
    source          VARCHAR(32) NOT NULL,          -- 'rss_bbc' | 'reddit_soccer' | 'newsapi'
...
matchbook:
  ...
  commission_pct: 0.04
  session_ttl_seconds: 600     # Refresh before 15min inactivity timeout
  max_retries: 3
  timeout_seconds: 10
...
9. `src/exchanges/matchbook.py`: full implementation:
   - `login()`: `POST /security/session` → store session-token in Redis with TTL=600s
...
> **Goal:** System places real bets on Matchbook. LLM analyzes news. Full risk management. 4-week paper validation begins.

**Tasks:**

1. `src/execution/matchbook_executor.py`:
   - `place_bet(side, event_id, market_id, runner_id, odds, stake)`:
     - `POST /offers` with `{odds, stake, side, runner-id}`
     - Poll `GET /offers/{id}` every 2s for up to 30s
     - Status `matched` → record in `bets`, Telegram confirmation
     - Status `unmatched` after 30s → `DELETE /offers/{id}`, record as cancelled
...
4. `src/data/news_aggregator.py`:
   ...
   - Reddit: `asyncpraw`. Subreddits: r/soccer, r/sportsbook + sport-specific subs
   - NewsAPI: `/v2/everything?q={sport}+{league}&sortBy=publishedAt`
...
5. `src/agents/situational_agent.py`:
   ...
   - On invalid JSON from Claude → log raw response, mark items as `analyzed=FALSE` (will retry next cycle), skip batch
...
7. **4 weeks of paper trading with full system running:**
   - All 3 agents active (arb + value + LLM)
...
| Matchbook 401 (session expired) | Re-authenticate (`POST /security/session`). Use `asyncio.Lock` — one re-auth at a time. Retry original request once. | Automatic | Log warning only. Alert if re-auth itself fails. |
...
| Claude API failure | Skip situational analysis for this cycle. Arb + value agents continue independently. | Auto-retry next cycle | Telegram after 5 consecutive failures |
| Claude returns invalid JSON | Log raw response. Mark news items as `analyzed=FALSE` (will retry next cycle). | Automatic retry | Log warning. Alert if >10 failures/day. |
...
│   │   └── situational_agent.py   # Claude Haiku news analysis agent
│   ├── engine/
│   │   ├── __init__.py
...
│   │   ├── news_aggregator.py     # RSS + Reddit + NewsAPI pipeline
...
**Auth flow:**
1. `POST /security/session` with `{"username": "...", "password": "..."}`
2. Response includes `session-token` header
3. Include `session-token` in all subsequent requests
4. Session expires after ~15 min inactivity
...
|---|---|---|---|
| POST | `/security/session` | Login | Returns session-token header |
| GET | `/events` | List events | Params: `sport-ids`, `after`, `before` |
| GET | `/events/{eid}/markets` | Markets for event | |
| GET | `/events/{eid}/markets/{mid}/runners` | Prices | Back/lay odds + liquidity |
| POST | `/offers` | Place bet | Body: `{odds, stake, side, runner-id}` |
| GET | `/offers/{oid}` | Check offer status | Poll for matching |
| DELETE | `/offers/{oid}` | Cancel unmatched | |
| GET | `/bets?status=open` | List open bets | Used by /kill |

**Rate limits:** Free under 1M GET/month (Trader Plan). Over 1M: £100/million.
...
### 11.3 Claude Haiku 4.5 (Anthropic)
...
**Cost:** ~$0.25/M input, ~$1.25/M output. Budget ~50K tokens/day ≈ €0.15/day.
...
**Fee:** 2% on winning outcome + ~$0.007 gas
...
| Claude Haiku 4.5 API | €5 | €20 | ~50K tokens/day budget |
| The Odds API | €0 | €35 | Free tier for Sprint 1–2 |
| NewsAPI.org | €0 | €45 | Free for development |
| Reddit API + RSS + Telegram | €0 | €0 | All free |
| **TOTAL INFRASTRUCTURE** | **€9** | **€108** | |
